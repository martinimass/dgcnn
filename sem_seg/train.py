...
parser.add_argument('--valid_area', type=int, default=0, help='Which area to use for validation, option: 1-6 [default: 6]')
...
if FLAGS.valid_area !=0:
  valid_data = data_batches[valid_idxs,...]
  valid_label = label_batches[valid_idxs]
...
def train():
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      #for i in xrange(FLAGS.num_gpu):  #PYTHON 2
      for i in range(FLAGS.num_gpu):  #PYTHON 3
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
            ...
            pred = get_model(pointclouds_phs[-1], is_training_phs[-1], bn_decay=bn_decay, classes=NUM_CLASSES)
